...
QMT_Server_Response* get_qmt_server_response_l(QString uri)
{
 ...
 static QString socket_file_path;
 if(socket_file_path.isEmpty())
 {
  log_comments_qstring("-qmt-server-response", "Looking for sockets in: %1\n"_qt
    .arg(PREFERRED_SOCKET_FOLDER));

  QDirIterator dit(PREFERRED_SOCKET_FOLDER, {"*.sock-info.txt"});

  if(dit.hasNext())
    dit.next();
  else
    log_comments_qstring("-qmt-server-response", "Found nothing!\n");

  socket_file_path = dit.filePath();
  if(socket_file_path.isEmpty())
    return nullptr;

  log_comments_qstring("-qmt-server-response", "Using socket info path: %1\n"_qt
    .arg(socket_file_path));
  socket_file_path.chop("-info.txt"_qt.length());

  QFileInfo qfi(socket_file_path);
  socket_file_path = qfi.absoluteFilePath();

  log_comments_qstring("-qmt-server-response", "Using socket path: %1\n"_qt
    .arg(socket_file_path));
 }

 qls.setServerName(socket_file_path);
...
QMT_Server_Response* get_qmt_server_response(QString uri, QString log_file) //, QString log_file = QString())
{
 if(uri.startsWith("-l"))
 {
  QMT_Server_Response* qsr = get_qmt_server_response_l(uri.mid(2));
  if(qsr)
    return qsr;
  log_comments_qstring("-qmt-server-response", "Unexpected failure to construct "
    "QMT_Server_Response object; reverting to non-isolated mode\n");
  uri = uri.mid(2);
 }
...
QMap<QString, QString>* get_external_response_qmap_socket(QString msg, QByteArray& result)
{
 ...
 qls.setServerName("/home/nlevisrael/gits/osm/mod-tile/qt-pro/mod_tile/-qt_/run-logs/test.sock");
...
int get_canned_response(void* url, char** response)
{
 ...
 QString file = "/home/nlevisrael/gits/osm/apache/install/htdocs/example-map/mapnik.xml";
